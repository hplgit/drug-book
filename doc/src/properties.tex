...
Parameter values for the model (\ref{asde1D100}) and (\ref{M100}) (copied from the Table in Section \ref{sec:consrate} ) %\ref{tab:nodrug}). %\label{tab:nodrug_again}
...
We derived closed state and open state blockers with the parameters summarized from the Table in Section \ref{sec:co-mutation}. %\ref{tab:drug}. 
