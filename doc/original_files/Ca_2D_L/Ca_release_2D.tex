\chapter{Two-dimensional calcium release \label{Ca_release_2D}}
...
\end{equation}
%\GTL{Added 'and', as it could be read as $c_{1}\gg v_r,v_d,v_s$, which we do not assume I guess}.
...
\fig{2D/mc2.pdf}{Results of simulation using the scheme ($\ref{nc1}$) and ($\ref{nc2}$) with the data given in Table \ref{tab:param2D}.}

\fig{2D/mc2_zoom.pdf}{A detailed view of the results given in Figure \ref{2D/mc2.pdf}. The open/closed state of the channel is indicated in the upper panel.}
...
\graytable{l}{
{|c|c|} \hline
$\Delta t $ & 0.001 $\rm{ms}$ \\ \hline
$\Delta x $ & 0.92 $\rm{\mu M}$\\ \hline
$\Delta y $ & 9.3 $\rm{\mu M}$ \\ \hline
...
\subsection{Rapid decay to steady state solutions in 2D \label{rapid2D}}
...
\fig{2D/pdf_time.pdf}{Open probability density function $\rho_o$ as a function of the dyad ($x$) and the JSR concentrations ($y$) for times $t=1$, 2, 3, 5, 10, 20, 40, 70, and 100 ms. Note the convergence toward
...
$\rho_o$ at time $t$ and at time $1000$ ms. This figure shows convergence toward an equilibrium solution.}

% {\it Glenn: Make two figures: 1) A figure with open probability in a 3x3 panels display of solutions at 9 different times. Illustrate decay to equilibrium. 2) Relative norm (L2) difference between time del solution and steady state solution as function of time. Use 2D version of the norm given in \ref{norm}}.
...
\fig{2D/koc3.pdf}{The effect of increasing $k_{oc}$ from one to three. The open probability density function is reduced considerably. }
...
\fig{2D/mutant.pdf}{The open state probability density function for the wild type case (left) and the mutant case (right, $\mu=3$). }
...
\fig{2D/closed_blocker.pdf}{Closed state blocker applied to the mutant case ($\mu=3$). As the value
$k_{bc}$ increases, the probability density function approaches the wild type solution. }

In Figure \ref{2D/closed_blocker.pdf} we show 
...
\fig{2D/closed_error.pdf}{The solution with the closed state blocker approaches the wild type case as $k_{bc}$ increases.}
...
\fig{2D/mu3.pdf}{Relative difference  between the wild type and the mutant with 
an open state blocker for the case $\mu=3$. There is a minimum around $(k_{bo},k_{ob})\approx(0.3,0.3)\text{ ms}^{-1}$ marked by a small $\times$.}
...
\fig{2D/mu10.pdf}{Relative difference between the wild type and the mutant with 
an open state blocker for the case $\mu=10$. There is a minimum around $(k_{bo},k_{ob})=(0.53,0.63)\text{ ms}^{-1}$.}
...
\fig{2D/optimal.pdf}{Open probability density function for the wild type, the mutant ($\mu=3$), the mutant plus the closed state blocker, and the mutant plus the open state blocker. We compute the stationary solution by solving the time-dependent equations until $T=100$ ms. In the computation we use $\Delta t=0.001$ ms, $\Delta x=0.92$ $\mu$M,  and  $\Delta y=9.3$ $\mu$M. %{\bf  GTL: why do we provide $\Delta t$ for a stationary solution?}
%\GTL{We simuluate untill steady state, T = 100ms. Direct steady state solve has not been implemented for 2D.}
 The model parameters are specified in Table \ref{tab:param2D_again}.  
...
%\K{zzz Er det Figure \ref{2D/mc_drug2.pdf}?}
...
\fig[1.0]{2D/mc_drug2.pdf}{Stochastic simulation of dyad concentrations (left, $x=x(t)$) and JSR concentrations
